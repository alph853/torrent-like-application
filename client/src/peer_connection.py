...
class PeerConnection:
    ...
    def handle_extension_message(self, message):
        ...
        if decoded_payload.get(b"metadata_size") is None:
            self.handle_extension_handshake_request()
        elif decoded_payload.get(b"m") is not None:
    ...
    def send_metadata_request(self):
        ...
        if piece_idx is not None:
            ...
            payload = MagnetUtils.construct_extension_payload(bencoded_request, self.extension_message_id)
            self.sock.send(payload)
        else:
    ...
    def handle_metadata_request(self, message):
        """Handle a metadata request message from the peer."""
        ...
        if metadata_piece is not None:
            response = {"msg_type": ExtensionMessageType.DATA.value, "piece": piece_index}
            bencoded_response = bencodepy.encode(response)
            payload = MagnetUtils.construct_extension_payload(
                bencoded_response, self.extension_message_id) + metadata_piece
            self.sock.send(payload)
        else:
            response = {"msg_type": ExtensionMessageType.REJECT.value, "piece": piece_index}
            bencoded_response = bencodepy.encode(response)
            payload = MagnetUtils.construct_extension_payload(bencoded_response, self.extension_message_id)
            self.sock.send(payload)
    ...
    def send_have_message(self, piece_index):
        ...
        message = struct.pack(">IBI", 5, MessageType.HAVE.value, piece_index)
        self.enqueue_send_message(message)
    ...
    def send_piece_message(self, piece_index, begin, block):
        ...
        message = struct.pack(">IBII", len(block) + 9, MessageType.PIECE.value, piece_index, begin) + block
        self.enqueue_send_message(message)
    ...
    def handle_interest_message(self):
        ...
        if self.id in self.piece_manager.get_unchoked_peers():
    ...
    def send_not_interested_message(self):
        ...
        message = struct.pack(">IB", 1, MessageType.NOT_INTERESTED.value)
        self.enqueue_send_message(message)
